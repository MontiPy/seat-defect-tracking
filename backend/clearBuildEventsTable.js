...
db.serialize(() => {
  ...
  db.run("DELETE FROM build_events", function(err) {
    if (err) {
      return console.error("Error clearing build_events:", err.message);
    }
    console.log(`Cleared ${this.changes} build_event record(s).`);
  });

  // Reset AUTOINCREMENT (optional)
  db.run("DELETE FROM sqlite_sequence WHERE name='build_events';", err => {
